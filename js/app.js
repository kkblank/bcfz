...
function getHeaderTitle(view, params) {
  ...
  if (view === 'category') {
    if (params.catName) return decodeURIComponent(params.catName);
     return params.type === 'formula' ? '方剂分类' : params.type === 'internal' ? '病症分类' : params.type === 'acupoint' ? '经脉分类' : params.type === 'acupuncture' ? '章节分类' : '草药分类';
  }
  if (view === 'list') {
...
function renderBreadcrumb(view, params) {
  ...
  let typeName = params.type === 'formula' ? '方剂学' : params.type === 'internal' ? '内科学' : params.type === 'acupoint' ? '腧穴学' : '中药学';
  if (params.type === 'acupuncture') typeName = '针灸学';
